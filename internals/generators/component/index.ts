...
export const componentGenerator: PlopGeneratorConfig = {
  ...
  prompts: [
    ...
    {
      type: 'confirm',
      name: ComponentProptNames.wantFold,
      message: 'Do you need to put the files in a folder?',
      default: true,
    } as any,
    {
      type: 'confirm',
  ...
  actions: (data) => {
    ...
    const fileName = answers.componentName.replace(/(?=[A-Z])([A-Z])/g, (str, $1, idx) => {
      return (idx !== 0 ? '_' : '') + $1.toLowerCase();
    });
    const pathName = answers.wantFold ? `/${fileName}` : '';
    const componentPath = `${baseGeneratorPath}/${answers.path}${pathName}`;
    ...
    const actions: Actions = [
      {
        type: 'add',
        path: `${componentPath}/${answers.wantFold ? 'index' : answers.componentName}.tsx`,
        templateFile: './component/index.tsx.hbs',
        abortOnFail: true,
      },
    ];

    if (answers.wantLoadable && answers.wantFold) {
      actions.push({
        type: 'add',
    ...
    if (answers.wantTests) {
      actions.push({
        type: 'add',
        path: `${componentPath}/__tests__/${
          answers.wantFold ? 'index' : answers.componentName
        }.test.tsx`,
    ...
    if (answers.wantTranslations) {
      actions.push({
        type: 'add',
        path: `${componentPath}/${answers.wantFold ? '' : answers.componentName + '.'}messages.ts`,
